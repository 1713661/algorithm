...
# 근데 왜 도착점부터 탐색해야하지? 출발점부터 탐색하면 안되나?
def bfs():
    q = deque()
    q.append(d) #도착점
    while q:
        ...
        if v == s:  # 시작점 도달
            continue 
        for pre_v, pre_c in r_graph[v]: # reverse graph니까 v->pre_v=pre_c라고 생각해야함
            if distance[pre_v] + graph[pre_v][v] == distance[v]: #로직은 백트래킹아니고 순트래킹(?). 생각을 해 생각을!!!
...
if __name__ == "__main__":
    while True:
        ...
        r_graph = [[] for _ in range(n)]               # reverse_graph
        for _ in range(m):
